  ...
  const options = {
    ...
    quantization: {
      ...
      items: [
        ...
        { id: 'w8a8', label: 'W8A8 (INT8)', default: false, disabledWhen: (values) => values.hardware !== 'xeon', disabledReason: 'W8A8 is only available on Xeon' }
      ]
    },
    toolcall: {
      name: 'toolcall',
  ...
  const generateCommand = () => {
    ...
    if (isXeon) {
      if (quantization === 'w8a8') {
        ...
        modelPath = `meta-llama/Llama-3.1-${sizeToken}-FP8`;
      } else if (quantization === 'bf16') {
        modelPath = `meta-llama/Llama-3.1-${sizeToken}${categorySuffix}`;
      }
    } else if (quantization === 'fp8' && category === 'instruct') {
      if (modelsize === '405b') {
    ...
    if (isAMD) {
      ...
      if (modelsize === '8b') tpSize = 1;
      else if (modelsize === '70b') tpSize = 2;
      else if (modelsize === '405b') tpSize = 4;
    } else {
    ...
    if (!isXeon && quantization === 'fp8' && category !== 'instruct') {
          ...
          <div style={itemsStyle}>
            {option.type === 'checkbox' ? (
              ...
              option.items.map(item => {
                ...
                const isDisabled = item.disabledWhen ? item.disabledWhen(values) : false;
                return (
                  <label key={item.id} style={{ ...labelBaseStyle, ...(isChecked ? checkedStyle : {}), ...(isDisabled ? disabledStyle : {}) }}>
